...
users = users + double_letters

for user in users:
    num_follows = random.randint(1, 702)  # Each user follows between 1 and 702 people
...
df.to_csv("follows_data.csv", index=False)
print("Total follow relationships:", cur.execute("SELECT COUNT(*) FROM follows").fetchone()[0])
print("===============================================")
start_time = time.time()
print(f"1. {person} follows:", who_a_follows)
end_time = time.time()
print(f"Query executed in: {end_time - start_time:.6f}s")
start_time = time.time()
print(f"2. Who follows {person}:", who_follows_a)
end_time = time.time()
print(f"Query executed in: {end_time - start_time:.6f}s")
start_time = time.time()
print(f"3. Mutual follows with {person}:", mutuals)
end_time = time.time()
print(f"Query executed in: {end_time - start_time:.6f}s")
start_time = time.time()
print(f"4. {person} follows but not followed back:", a_one_way)
end_time = time.time()
print(f"Query executed in: {end_time - start_time:.6f}s")
start_time = time.time()
print(f"5. Follows {person} but {person} doesn’t follow back:", follows_a_only)
end_time = time.time()
print(f"Query executed in: {end_time - start_time:.6f}s")
